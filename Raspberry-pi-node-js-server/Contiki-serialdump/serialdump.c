...
int main(int argc, char **argv)
{
  ...
  char christos[BUFSIZ];
  int christos_counter = 0;
  ...
  while (index < argc) {
	...
	if(strlen(&argv[index][2]) == 0) {
	  //Change datetime format to be the same as DateTime object on javascript
...
#ifndef __APPLE__
  fd = open(device, O_RDWR | O_NOCTTY | O_NDELAY | O_CREAT | O_SYNC );
#else
...
/* 	} */
	...
	case MODE_START_DATE: {
	  ...
	  //Do not print the buffer immediately
	  //printf("%s|", outbuf);
	  mode = MODE_DATE;
	}
	...
	case MODE_DATE:
	  // Do not print the buffer immediately
	  //printf("%c", buf[i]);
	  //Instead, use a new array
	  christos[christos_counter] = buf[i];
	  christos_counter++;
	  //When the buffer value is a new line
	  if(buf[i] == '\n') {
	    christos_counter = 0;
	    //print the values of date and the array
	    printf("%s", outbuf);
	    //print also the 000Z
	    printf(".000Z|");
	    printf("%s", christos);
	    //make array empty
	    for(christos_counter = 0; christos_counter < BUFSIZ; christos_counter ++)
	    {
	      christos[christos_counter] = 0;
	    }
	    christos_counter = 0;
	    mode = MODE_START_DATE;
	  }
